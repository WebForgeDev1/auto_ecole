...
if (contactForm && formNote) {
  contactForm.addEventListener("submit", (event) => {
    ...
    const name = String(formData.get("firstname") || formData.get("name") || "").trim();
...
if (calendlyEmbed && calendlyLoadBtn) {
  const calendlyMobile = window.matchMedia("(max-width: 640px)");

  if (calendlyMobile.matches) {
    calendlyLoadBtn.textContent = "Choisir mon créneau";
  }
...
document.querySelectorAll(".carousel-wrap").forEach((wrap) => {
  const track = wrap.querySelector(".review-grid-static, .team-grid, .formula-grid, .formula-grid--trio");
